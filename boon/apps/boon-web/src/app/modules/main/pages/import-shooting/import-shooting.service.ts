...
import { FileUploadsApiClient, TemplateApiClient } from '../../../../../api/services';
...
export class ImportShootingService {
  private readonly idFunzionalita = 1; // TODO:: mappa funzionalità
  ...
  getTemplatesForImportShooting() {
    const params = PaginationQueryString.builder()
      .addFilter('funzionalita.id', ['$eq'], this.idFunzionalita)
      .finalize()
      .paramsify();
  ...
  uploadFile(template: TemplateResExcerptDto, file: File) {
    ...
    return this.fileUploadsApiClient.fileUploadsControllerCreateFileUpload(params).pipe(
      tap(() => {
        this.messageService.add({
          severity: 'success',
  ...
  async downloadExcel(data: any) {
    const now = new Date();
    const fileName = 'Import-shooting_' + dayjs(now).format('DD-MM-YYYY');
    const title = 'Import-shooting_' + dayjs(now).format('DD-MM-YYYY');
