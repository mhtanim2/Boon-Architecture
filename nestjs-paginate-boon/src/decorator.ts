...
import { pickBy, Dictionary, isString, mapKeys, chain, first, isArray } from 'lodash'
...
export interface PaginateQuery {
    ...
    orFilters?: { [column: string]: string | string[] }[]
    select?: string[]
    path: string
}
...
export const Paginate = createParamDecorator((_data: unknown, ctx: ExecutionContext): PaginateQuery => {
    ...
    const groupRegex = new RegExp(/^\[(\d+)\]/)
    const filtersRaw = chain(
        Object.entries(filters).flatMap(([key, value]) =>
            (isArray(value) ? value : [value]).map((y) => ({ key, value: y }))
        )
    )
        .map(({ key, value: _value }) => {
            const group = _value.match(groupRegex)?.[1] ?? -1
            const value = _value.replace(`[${group}]:`, '')
            return { key, group, value }
        })
        .groupBy((x) => x.group)
        .mapValues((grouping, group) =>
            chain(grouping)
                .groupBy((x) => x.key)
                .mapValues((v) => {
    ...
    const andFilters = filtersRaw[-1] ?? {}
    const orFilters = chain(filtersRaw)
        .pickBy((_, name) => name !== '-1')
        .map()
        .value()
    ...
    return {
        ...
        searchBy,
        filter: Object.keys(andFilters).length ? andFilters : undefined,
