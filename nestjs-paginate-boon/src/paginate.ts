...
import { addFilter, addOrFilters, FilterOperator, FilterSuffix } from './filter'
...
export async function paginate<T extends ObjectLiteral>(
    ...
    if ((query.orFilters ?? []).length > 0) {
        addOrFilters(queryBuilder, query, config.filterableColumns)
    }
